...
function urlCheck(url) {
    var targetTestURL = 'https://www.swyftfilings.com/';
    if (window.location.pathname === "/marketing/llc/" || window.location.pathname === "/") {
        targetTestURL = window.location.href;
    }
    if (isSameUrl(url, targetTestURL, true) || isSameUrl(url, targetTestURL + '/marketing/llc/', true)) {
        createTest();
    } else {
        removeTest();
    }
...
function createTest() {
    ...
    var bodyIntervalInit = setInterval(function () {
        var bodyElement = document.querySelector('body');
        if (bodyElement) {
            clearInterval(bodyIntervalInit);
            ...
            waitForElm('body ' + heroSelector + ' .hero-wrap').then(function () {
                let heroHTML = setInterval(() => {
                    if (window.location.pathname === "/" || window.location.pathname === "/marketing/llc/") {
                        if (document.body && !document.querySelector('body ' + heroSelector + ' .spz-wrap')) {
                            document.querySelector('body ' + heroSelector + '').insertAdjacentHTML('beforeend', `<div class="spz-wrap"><div class="spz-row"><div class="spz-col left-copy">
            ...
            <div class="swiper-btn-next"><svg xmlns="http://www.w3.org/2000/svg" width="33" height="32" viewBox="0 0 33 32" fill="none">
                <rect x="-0.5" y="0.5" width="31" height="31" rx="15.5" transform="matrix(-1 0 0 1 31.6667 0)" stroke="#FAFCFF"></rect>
                <path d="M18.7971 22.0286L12.7685 16L18.7971 9.97131" stroke="#FAFCFF" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg></div>
            <div class="swiper-btn-prev"><svg xmlns="http://www.w3.org/2000/svg" width="33" height="32" viewBox="0 0 33 32" fill="none">
                <rect x="1.16675" y="0.5" width="31" height="31" rx="15.5" stroke="#FAFCFF"></rect>
                <path d="M14.5364 22.0286L20.565 16L14.5364 9.97131" stroke="#FAFCFF" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg></div>
        </div>
            </div>
        <div class="spz-col right-img"></div></div></div>`);
                            ...
                            if (window.location.pathname.indexOf("/marketing/llc/") > -1 && !document.querySelector('.spz-price-section')) {
...
//Generic code
function removeTest() {
    if (document.body.classList.contains('.spz-1101')) {
        document.body.classList.remove('.spz-1101');
    }
    if (document.querySelector('.spz-wrap')) {
        document.querySelector('.spz-wrap').remove();
    }
    if (document.querySelector('.spz-price-section')) {
        document.querySelector('.spz-price-section').remove();
    }
}

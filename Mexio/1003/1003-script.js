		...
		<div class="spz-bg-wrap">
			...
			<div class="spz-email">
						 <div class="form-group-wrapper">
                                ...
                                <a href="${formData.heroCTA.CTAHref}" id="hero-copy-url" class="hero-btn redirect-to-demo spz_tracking_1003">
...
function formPage() {
	var iframebodyInterval = setInterval(function () {
		...
		if (iframebodyEle) {
			if (!iframebodyEle.classList.contains('spz-1003-iframe')) {
				...
				waitForElm('body #pardot-form').then(function () {
					...
					if (formAction.includes('?')) {
						document.querySelector('body #pardot-form').action = formAction + '&spz=1003';
					} else {
						document.querySelector('body #pardot-form').action = formAction + '?spz=1003';
					}
				});
...
function demoPage() {
	var bodyInterval = setInterval(function () {
		...
		if (!bodyEle.classList.contains('spz-1003-demo')) {
			clearInterval(bodyInterval);
			bodyEle.classList.add('spz-1003-demo');
			waitForElm('.blocks-page iframe').then(function () {
				setTimeout(() => {
					var iframeSrc = document.querySelector('.blocks-page iframe').src;
					if (iframeSrc.includes('?')) {
						document.querySelector('.blocks-page iframe').src = iframeSrc + '&spz=1003';
					} else {
						document.querySelector('.blocks-page iframe').src = iframeSrc + '?spz=1003';
					}
				}, 500);
...
if (location.href.indexOf('spz=1003') > -1) {
	formPage();
}
...
/**
 * Code of Maxio 1003 Starts from Here
 */
...
//DEV 5/7. Set unique selector of target setion where you want review ribbon to appear (updated selector from .spz-1003 main#blocks-page > section:nth-child(2) because of Maxio 1002 code conflict)
const sectionSelector = `.spz-1003 main#blocks-page > section:nth-of-type(2)`

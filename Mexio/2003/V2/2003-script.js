...
function formPage() {
    var iframebodyInterval = setInterval(function () {
        ...
        if (iframebodyEle) {
            if (!iframebodyEle.classList.contains('spz-2003-v2-iframe')) {
                clearInterval(iframebodyInterval);

                iframebodyEle.classList.add('spz-2003-v2-iframe');
                ...
                waitForElm('.spz-2003-v2-iframe form#pardot-form .submit input').then(function () {
                    document.querySelector(".spz-2003-v2-iframe form#pardot-form").insertAdjacentHTML("beforebegin", `<div class="form-heading" style="display: none;">Get a Demo</div>`);

                    document.querySelector('.spz-2003-v2-iframe form#pardot-form .country').classList.add('spz-hidden');
                    document.querySelector('.spz-2003-v2-iframe form#pardot-form .Employee_Count').classList.add('spz-hidden');
                    document.querySelector('.spz-2003-v2-iframe form#pardot-form .pd-checkbox').classList.add('spz-hidden');

                    hiddenValue();
                    var all_inputs = document.querySelectorAll('.spz-2003-v2-iframe form#pardot-form input, .spz-2003-v2-iframe form#pardot-form select');
                    all_inputs.forEach(function (element) {
                        ...
                        if (element.tagName == 'SELECT' && element.selectedIndex !== 0 && element.closest('.form-field') !== null) {
                            element.closest('.spz-2003-v2-iframe form#pardot-form .form-field').classList.add('filled');
                        }
                        if (element.tagName !== 'SELECT' && element.value != '' && element.closest('.form-field') !== null) {
                            element.closest('.spz-2003-v2-iframe form#pardot-form .form-field').classList.add('filled');
                        }
                    });
                    var selector = '.spz-2003-v2-iframe form#pardot-form .form-field input, .spz-2003-v2-iframe form#pardot-form .form-field select';
                    document.addEventListener('focus', function (event) {
                        if (event.target.matches && event.target.matches(selector)) {
                            event.target.closest('.spz-2003-v2-iframe form#pardot-form .form-field').classList.add('active', 'typing');
                        }
                    }, true);
                        ...
                        document.addEventListener(s_event, function (event) {
                            ...
                            if (event.target.matches && event.target.matches(selector)) {
                                if (event.target.value == null || event.target.value == '') {
                                    event.target.closest('.spz-2003-v2-iframe form#pardot-form .form-field').classList.remove('filled');
                                } else {
                                    if (event.target.tagName == 'SELECT' && event.target.selectedIndex !== 0) {
                                        event.target.closest('.spz-2003-v2-iframe form#pardot-form .form-field').classList.add('filled');
                                    }
                    ...
                    document.addEventListener('focusout', function (event) {
                        document.querySelectorAll('.spz-2003-v2-iframe form#pardot-form .form-field.typing').forEach(function (elem) {
                ...
                function checkFormSubmit() {
                    ...
                    var chekFormSubmit = setInterval(function () {
                        if (document.querySelector('.spz-2003-v2-iframe form#pardot-form .submit input') == null) {
                            clearInterval(chekFormSubmit);
                            document.querySelectorAll('.spz-2003-v2-iframe form#pardot-form > p').forEach((PEle) => {
                                if (PEle.textContent == 'We look forward to speaking with you soon!') {
                                    if (document.querySelector('.spz-2003-v2-iframe .form-heading')) {
                                        document.querySelector('.spz-2003-v2-iframe .form-heading').style.display = "none";
                                    }
                                }
                            });
                        }
                ...
                function checkFilledVisibility() {
                    if (document.querySelector('.spz-2003-v2-iframe form#pardot-form .form-field.first_name').classList.contains('filled') && document.querySelector('.spz-2003-v2-iframe form#pardot-form .form-field.last_name').classList.contains('filled') && document.querySelector('.spz-2003-v2-iframe form#pardot-form .form-field.email').classList.contains('filled') && document.querySelector('.spz-2003-v2-iframe form#pardot-form .form-field.company').classList.contains('filled')) {

                        document.querySelectorAll('.form-field.spz-hidden').forEach(function (elem) {
                            elem.classList.remove('spz-hidden');
                        })

                        document.querySelector('.spz-2003-v2-iframe form#pardot-form ').classList.add('spz-full-form');
                    }
                }
            }
        }
...
function demoPage() {
    var bodyInterval = setInterval(function () {
        ...
        if (!bodyEle.classList.contains('spz-2003-v2')) {
            ...
            cookieValue('#2003', '#2003_variant2');

            if (!document.querySelector('body.spz-2003-v2 #page .site-content .blocks-page .hero-form .container .spz-form-wrap')) {
                document.querySelector('body.spz-2003-v2 #page .site-content .blocks-page .hero-form .container').insertAdjacentHTML('beforeend', `<div class="spz-form-wrap"><div class="form-heading">Get a Demo</div></div><div class="spz-2003-v2-right-section">
                <div class="rs-inner">
                ...
                if (spz_2003_iframe) {
                    ...
                    if (formHeight) {
                        if (formHeight.includes('height: 40.3958px') || formHeight.includes('height: 62.7917px') || formHeight.includes('height: 22.3958px') || formHeight.includes('height: 44.7917px')) {
                            ...
                            document.querySelector('.spz-2003-v2 .spz-form-wrap .form-heading').style.display = "none";
                        }
                    }
                }
            }, 200);

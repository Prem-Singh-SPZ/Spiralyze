...
function formPage() {
	var iframebodyInterval = setInterval(function () {
		...
		if (iframebodyEle) {
			if (!iframebodyEle.classList.contains('spz-1002-iframe')) {
				...
				waitForElm('body #pardot-form').then(function () {
					...
					if (formAction.includes('?')) {
						document.querySelector('body #pardot-form').action = formAction + '&spz=1002';
					} else {
						document.querySelector('body #pardot-form').action = formAction + '?spz=1002';
					}
				});
...
function demoPage() {
	var bodyInterval = setInterval(function () {
		...
		if (!bodyEle.classList.contains('spz-1002-demo')) {
			...
			waitForElm('.blocks-page iframe').then(function () {
				setTimeout(() => {
					...
					if (iframeSrc.includes('?')) {
						document.querySelector('.blocks-page iframe').src = iframeSrc + '&spz=1002';
					} else {
						document.querySelector('.blocks-page iframe').src = iframeSrc + '?spz=1002';
					}
				}, 500);
...
if (location.href.indexOf('spz=1002') > -1) {
	formPage();
}

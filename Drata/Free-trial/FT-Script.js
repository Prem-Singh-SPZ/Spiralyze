const formInt = setInterval(() => {
    if (document.querySelectorAll('.hbspt-form form').length > 0) {
        ...
        document.querySelector('label#label-demo_product_of_interest-ab3d7d8d-c892-4853-93a7-b0f811a767b5 + .hs-field-desc').innerHTML = 'What product(s) are you interested in?';
...
function forClearBitForms() {
    document.querySelectorAll('.hs-input').forEach(function (el) {
        if (el.closest('.hs-form-field[style*="display: none"]')) {
            if (!el.closest('fieldset').classList.contains('form-columns-2')) {

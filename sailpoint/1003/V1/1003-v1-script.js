(function () {
  function createTest() {
    let bodyLoaded = setInterval(function () {
      ...
      if (body) {
        ...
        if (!body.classList.contains('spz-1003-v1')) {
          body.classList.add('spz-1003-v1');
          ...
          document.querySelector('#__next footer').insertAdjacentHTML('beforebegin', `<div class="spz-form-wrap">
            ...
            document.querySelector('#__next footer').insertAdjacentHTML('afterbegin', `<div class="spz-review-disclaimer">
          ...
          hiddenValue('spz-1003', 'SPZ_1003_variant_1');

          function formModify() {
            // Add class in mktoFormRow using count
            var form_fields = document.querySelectorAll('.spz-1003-v1 .spz-form-section form.mktoForm .mktoFormRow');
            ...
            ['#LblCountry:Country', '#LblState:State', '#LblNumber_of_Employees__c:Number of employees'].forEach(item => {
              ...
              waitForElm(`.spz-1003-v1 .spz-form-section form.mktoForm .mktoFieldWrap label.mktoLabel${id}`).then(label => {
                label.innerHTML = (label.querySelector('.mktoAsterix')?.outerHTML || '') + text;
              });
            });

            waitForElm('.spz-1003-v1 .spz-form-section form.mktoForm .mktoFormRow.field-31 .mktoCheckboxList label').then(label => {
              label.textContent = "Uncheck to stop receiving SailPoint email communications.";
            });

            // Change Field Position
            var employees_field = document.querySelector('.spz-1003-v1 form.mktoForm .field-8');
            var state_field = document.querySelector('.spz-1003-v1 form.mktoForm .field-10');
            state_field.after(employees_field);

            if (window.innerWidth > 767) {
              var email_field = document.querySelector('.spz-1003-v1 form.mktoForm .field-4');
              var phone_field = document.querySelector('.spz-1003-v1 form.mktoForm .field-5');
              phone_field.after(email_field);
            }

            var disclaimer_field = document.querySelector('.spz-1003-v1 form.mktoForm .mktoCaptchaDisclaimer');
            var button = document.querySelector('.spz-1003-v1 form.mktoForm .mktoButtonRow');
            button.after(disclaimer_field);

            document.querySelector('.spz-1003-v1 form.mktoForm .field-11 .mktoField').value = 'Get demo';

            waitForElm(`.spz-1003-v1 .spz-form-section form.mktoForm .mktoFieldWrap select#Country`).then((elm) => {
              setTimeout(() => {
                document.querySelectorAll(`.spz-1003-v1 form.mktoForm .mktoFormCol .mktoFieldWrap .mktoField`).forEach(function (el) {
            ...
            function focusFields() {
              ...
              const form = document.querySelector('.spz-1003-v1 form.mktoForm');
              ...
              function reapplyStateFieldListeners() {
                const stateField = document.querySelector('.spz-1003-v1 form.mktoForm .mktoFieldWrap .mktoField#State');
                if (stateField) {
              ...
              const container = document.querySelector('.spz-1003-v1 form.mktoForm');
              if (container) {
            ...
            function checkAllFields() {
              const fields = document.querySelectorAll('.spz-1003-v1 form.mktoForm .mktoField');
            ...
            document.querySelector('select#Country').addEventListener('change', () => {
              const stateRow = document.querySelector('.spz-1003-v1 .spz-form-section form.mktoForm .mktoFormRow.field-10');
              const optOutRow = document.querySelector('.spz-1003-v1 .spz-form-section form.mktoForm .mktoFormRow.field-31');
              const employeeNumberRow = document.querySelector('.spz-1003-v1 .spz-form-section form.mktoForm .mktoFormRow.field-8');
  ...
  function removeTest() {
    setTimeout(() => {
      if (document.querySelector('.spz-1003-v1')) {
        document.body.classList.remove("spz-1003-v1");
      }
    }, 2000);
  }

(function () {
  function createTest() {
    let bodyLoaded = setInterval(function () {
      ...
      if (body) {
        ...
        if (!body.classList.contains('SPZ-1008-TC')) {
          body.classList.add('SPZ-1008-TC');
          ...
          function addBaseline(formData, whereToPut, formSelector, heroSelector, additionalSection) {
            ...
            let formLoaded = setInterval(() => {
              if (document.querySelector(".SPZ-1008-TC .spz-form-wrap .the-form") && document.querySelector(formSelector) && document.querySelectorAll(`${formSelector} input`).length > 0) {
                clearInterval(formLoaded)
                document.querySelector(".SPZ-1008-TC .spz-form-wrap .the-form").appendChild(document.querySelector(formSelector));
                waitForElm('.SPZ-1008-TC .mkto-wrap + .disclaimer').then(() => {
                  document.querySelector(".SPZ-1008-TC .spz-form-wrap .the-form")?.appendChild(document.querySelector('.SPZ-1008-TC .mkto-wrap + .disclaimer')?.cloneNode(true));
                });
          ...
          let spzHeroInterval = setInterval(() => {
            if (document.querySelectorAll('.spz-hero').length == 0 && window.location.pathname === '/demo') {
              ...
              removeSpecificCookieValue('SPZ_1008', 'SPZ_1008_variant');
              hiddenValue('SPZ_1008', 'SPZ_1008_truecontrol');
              setHiddenFieldValue();
            }
            else {
          ...
          function formModify() {
            ...
            var form_fields = document.querySelectorAll('.SPZ-1008-TC .spz-form-section form.mktoForm .mktoFormRow');
            ...
            ['#LblCountry:Country', '#LblState:State', '#LblNumber_of_Employees__c:Number of employees'].forEach(item => {
              ...
              waitForElm(`.SPZ-1008-TC .spz-form-section form.mktoForm .mktoFieldWrap label.mktoLabel${id}`).then(label => {
                label.innerHTML = (label.querySelector('.mktoAsterix')?.outerHTML || '') + text;
              });
            });

            // waitForElm('.SPZ-1008-TC .spz-form-section form.mktoForm .mktoFormRow.field-31 .mktoCheckboxList label').then(label => {
            ...
            waitForElm(`.SPZ-1008-TC .spz-form-section form.mktoForm .mktoFieldWrap select#Country`).then((elm) => {
              setTimeout(() => {
                document.querySelectorAll(`.SPZ-1008-TC form.mktoForm .mktoFormCol .mktoFieldWrap .mktoField`).forEach(function (el) {
            ...
            function focusFields() {
              ...
              const form = document.querySelector('.SPZ-1008-TC form.mktoForm');
              ...
              function reapplyStateFieldListeners() {
                const stateField = document.querySelector('.SPZ-1008-TC form.mktoForm .mktoFieldWrap .mktoField#State');
                if (stateField) {
              ...
              const container = document.querySelector('.SPZ-1008-TC form.mktoForm');
              if (container) {
            ...
            function checkAllFields() {
              const fields = document.querySelectorAll('.SPZ-1008-TC form.mktoForm .mktoField');
            ...
            waitForElm('.SPZ-1008-TC .mkto-wrap.w-full #confirm #contact_us_submit').then(() => {
              document.body.classList.add('form-submit');
            });
  ...
  function removeTest() {
    setTimeout(() => {
      if (document.querySelector('.SPZ-1008-TC')) {
        document.body.classList.remove("SPZ-1008-TC");
      }
  ...
  function setHiddenFieldValue() {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.mktoForm .mktoButton')) {
        waitForElm('.SPZ-1008-TC #mktoForm_1018.mktoForm .mktoError').then(function (elm) {
          if (elm.parentNode.querySelector('#ValidMsgEmail')) {
            ...
            const intervalIdA = setInterval(() => {
              if (document.querySelector('.SPZ-1008-TC #mktoForm_1018.mktoForm .mktoError #ValidMsgEmail') !== null) {
                document.querySelector('.SPZ-1008-TC #mktoForm_1018.mktoForm .mktoError #ValidMsgEmail').parentNode.parentNode.classList.add('error');
              }
              counterA++;
              if (counterA >= 10) {

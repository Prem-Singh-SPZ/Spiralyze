(function () {
  ...
  function addCta() {
    if (document.querySelectorAll('.SPZ_7003 #page-container nav.navigation .desktop div.relative a.spz-contact-us').length == 0 && document.querySelector('.SPZ_7003 #page-container nav.navigation .desktop div.relative a[href="/demo"]')) {
      document.querySelector('.SPZ_7003 #page-container nav.navigation .desktop div.relative a[href="/demo"]').insertAdjacentHTML('afterend', `<a class="btn btn--outline spz-contact-us spz-btn-desk" href="javascript:;">Contact us</a>`);
      document.querySelector('.SPZ_7003 #page-container nav.navigation .mobile a[href="/demo"]').insertAdjacentHTML('afterend', `<a class="btn btn--outline p-2 text-sm spz-contact-us spz-btn-mob" href="javascript:;">Contact us</a>`);

      document.querySelectorAll('.SPZ_7003 #page-container .hero--homepage .hero__buttons .btn').forEach(function (el) {
  ...
  function formModify() {
    if (document.querySelector('.SPZ_7003 #page-container .flex.min-h-screen')) {
      waitForElm('.SPZ_7003 .mkto-wrap.w-full .mktoFormRow input').then(() => {
    ...
    var form_fields = document.querySelectorAll('.SPZ_7003 #mktoForm_1017.mktoForm .mktoFormRow');
    ...
    ['#LblCountry:Country', '#LblState:State', '#LblreasonforInquiry:Reason for inquiry'].forEach(item => {
      ...
      waitForElm(`.SPZ_7003 #mktoForm_1017.mktoForm .mktoFieldWrap label.mktoLabel${id}`).then(label => {
        label.innerHTML = (label.querySelector('.mktoAsterix')?.outerHTML || '') + text;
      });
    });

    waitForElm(`.SPZ_7003 #mktoForm_1017.mktoForm .mktoFieldWrap select#Country`).then((elm) => {
      setTimeout(() => {
        document.querySelectorAll(`.SPZ_7003 #mktoForm_1017.mktoForm .mktoFormCol .mktoFieldWrap .mktoField`).forEach(function (el) {
    ...
    function focusFields() {
      ...
      const container = document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm');
      if (container) {
    ...
    function checkAllFields() {
      const fields = document.querySelectorAll('.SPZ_7003 #mktoForm_1017.mktoForm .mktoField');
    ...
    if (document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm select#Country')) {
      document.querySelector('select#Country').addEventListener('change', () => {
        const stateRow = document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm .row_Country + .mktoFormRow');
        const optOutRow = document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm .mktoFormRow.row_reasonforInquiry');
        const countryRow = document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm .mktoFormRow.row_Country');
    ...
    if (typeof MktoForms2 !== 'undefined') {
      MktoForms2.whenReady(function (form) {
        form.onSuccess(function (values, followUpUrl) {
          document.body.classList.add('form-submit-7003');
        });
  ...
  function checkEmail() {
    const emailField = document.querySelector('.SPZ_7003 #mktoForm_1017.mktoForm input[name="Email"]');
    const emailRegex = /^[^\s@]+@[a-zA-Z0-9]+(?:\.[a-zA-Z0-9-]+)*\.[a-zA-Z]{2,}$/;
    if (emailField) {
  ...
  function removeTest() {
    setTimeout(() => {
      if (document.querySelector('.SPZ_7003')) {
        document.body.classList.remove("SPZ_7003");
      }
  ...
  document.addEventListener('click', function (e) {
    if (e.target.closest('#mktoForm_1017 .mktoButton')) {
      ...
      const fields = document.querySelectorAll('.SPZ_7003 form.mktoForm .mktoField');
      const timeBuffer = setInterval(() => {
        fields.forEach(field => {
          const fieldWrap = field.closest('.mktoFieldWrap');
          if (fieldWrap) {

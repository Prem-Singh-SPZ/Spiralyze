...
(function () {
  ...
  function contact() {
    let bodyLoaded = setInterval(function () {
      ...
      if (body) {
        clearInterval(bodyLoaded);
        if (!document.body.classList.contains('SPZ_7003_TC_Contact')) {
          document.body.classList.add('SPZ_7003_TC_Contact');
          const contactTrigger = sessionStorage.getItem('#7003_Contact');
          if (contactTrigger) {
            sessionStorage.removeItem('#7003_Contact');
            removeSpecificCookieValue('SPZ_7003', 'SPZ_7003_variant');
            hiddenValue('SPZ_7003', 'SPZ_7003_truecontrol');
          }
        }
  ...
  function removeTest() {
    setTimeout(() => {
      if (document.querySelector('.SPZ_7003_TC')) {
        document.body.classList.remove("SPZ_7003_TC");
      }
      if (document.querySelector('.SPZ_7003_TC_Contact')) {
        document.body.classList.remove("SPZ_7003_TC_Contact");
      }
    }, 200);
  }
  ...
  function urlCheck(url) {
    ...
    const currentURL = normalizeURL(window.location.href.split('?')[0]);

    if (urls.map(normalizeURL).includes(currentURL)) {
      createTest();
    } else if (currentURL === normalizeURL("https://www.sailpoint.com/contact-us")) {
      contact();
    } else {
      removeTest();
    }

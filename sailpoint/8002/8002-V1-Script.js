(function () {
  function createTest() {
    let bodyLoaded = setInterval(function () {
      ...
      if (body) {
        ...
        if (!body.classList.contains('spz_8002_v1')) {
          body.classList.add('spz_8002_v1');
          ...
          function addForm(formData, formSelector, whereToPut, templateSelector) {
            ...
            let formLoaded = setInterval(() => {
              if (document.querySelector(".spz_8002_v1 .spz-form-over-ui .the-form") && document.querySelector(formSelector) && document.querySelectorAll(`${formSelector} input`).length > 0) {
                clearInterval(formLoaded)
                document.querySelector(".spz_8002_v1 .spz-form-over-ui .the-form").appendChild(document.querySelector(formSelector));
                formModify();
              }
            });
          }
          ...
          let spzHeroInterval = setInterval(() => {
            if (document.querySelectorAll('.spz-form-over-ui').length == 0 && window.location.pathname === '/demo/interactive' && document.querySelector('.modal-form-block')) {
              ...
              removeSpecificCookieValue('SPZ_8002', ['SPZ_8002_truecontrol', 'SPZ_8002_variant2']);
              hiddenValue('SPZ_8002', 'SPZ_8002_variant1');
              setHiddenFieldValue();
            }
            else {
          ...
          function formModify() {
            var form_fields = document.querySelectorAll('.spz_8002_v1 .spz-form-over-ui form.mktoForm .mktoFormRow');
            for (var i = 0; i < form_fields.length; i++) {
              var dynamicClass = 'field-' + (i + 1);
              form_fields[i].classList.add(dynamicClass);
            }
            // var phone_field = document.querySelector('.spz_8002_v1 form.mktoForm .field-4');
            // var job_field = document.querySelector('.spz_8002_v1 form.mktoForm .field-6');
            // job_field.after(phone_field);

            waitForElm('.spz_8002_v1 form.mktoForm .disclaimer').then((elm) => {
              changePos();
            });
            waitForElm(`.spz_8002_v1 .spz-form-over-ui form.mktoForm .mktoFieldWrap select#Country`).then((elm) => {
              setTimeout(() => {
                document.querySelectorAll(`.spz_8002_v1 form.mktoForm .mktoFormCol .mktoFieldWrap .mktoField`).forEach(function (el) {
            ...
            function focusFields() {
              const form = document.querySelector('.spz_8002_v1 form.mktoForm');
              ...
              form.addEventListener('focus', function (event) {
                ...
                var email_field = document.querySelector('.spz_8002_v1 .spz-form-over-ui form.mktoForm input[name="Email"]');
              ...
              function reapplyStateFieldListeners() {
                const stateField = document.querySelector('.spz_8002_v1 form.mktoForm .mktoFieldWrap .mktoField#State');
                if (stateField) {
            ...
            document.querySelector('select#Country').addEventListener('change', () => {
              const stateRow = document.querySelector('.spz_8002_v1 .spz-form-over-ui form.mktoForm .mktoFormRow.field-8');
              const countryRow = document.querySelector('.spz_8002_v1 .spz-form-over-ui form.mktoForm .mktoFormRow.field-7');
        ...
        waitForElm('.spz_8002_v1 #mktoForm_1016.mktoForm .mktoButton').then(() => {
          document.querySelector('#mktoForm_1016.mktoForm .mktoButton').addEventListener('click', (event) => {
            ...
            waitForElm('.spz_8002_v1 .mktoForm .mktoError').then(function (elm) {
              if (elm.parentNode.querySelector('#ValidMsgEmail')) {
                ...
                const intervalIdA = setInterval(() => {
                  if (document.querySelector('.spz_8002_v1 .mktoForm .mktoError #ValidMsgEmail') !== null) {
                    document.querySelector('.spz_8002_v1 .mktoForm .mktoError #ValidMsgEmail').parentNode.parentNode.classList.add('error');
                  }
                  counterA++;
                  if (counterA >= 10) {
  ...
  function changePos() {
    const disclaimer_field = document.querySelector('.spz_8002_v1 form.mktoForm .mktoCaptchaDisclaimer');
    const button = document.querySelector('.spz_8002_v1 form.mktoForm .mktoButtonRow');
  ...
  function removeTest() {
    setTimeout(() => {
      if (document.querySelector('.spz_8002_v1')) {
        document.body.classList.remove("spz_8002_v1");
      }
      if (document.querySelector('.form-submit')) {
        document.body.classList.remove("form-submit");
      }
      if (document.querySelector('.spz-form-over-ui')) {
        document.querySelector('.spz-form-over-ui').remove();
      }
    }, 1000);
  }
  ...
  function checkEmail() {
    const emailField = document.querySelector('.spz_8002_v1 .spz-form-over-ui form.mktoForm input[name="Email"]');
    const emailRegex = /^[^\s@]+@[a-zA-Z0-9]+(?:\.[a-zA-Z0-9-]+)*\.[a-zA-Z]{2,}$/;
    if (emailField) {
  ...
  function urlCheck(url) {
    if (window.innerWidth >= 992) {
      ...
      let bodyLoaded = setInterval(function () {
        ...
        if (body) {
          clearInterval(bodyLoaded);
          if (!body.classList.contains('spz_8002_v1_mobile')) {
            body.classList.add('spz_8002_v1_mobile');
            removeTest();
          }
        }
      });

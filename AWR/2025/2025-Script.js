const cookieName = "#2031_AWR_Product_List_Popup";
const cookieValue = "1";
...
function createTest() {
    ...
    const productsCheck = setInterval(async function () {
        if (jQuery(".page-wrap.product-list .product-item .card").length != 0) {
            ...
            jQuery(".spz-2031 .product-list-area > .page-wrap.set-pad > .product-list-header .zipcode-box button.product-page-search > span:first-child").text("Search");
...
async function removeTest() {
    jQuery("body").removeClass("spz-2031");
    jQuery(".products-wrapper").remove();
    jQuery(".noproducts-wrapper").remove();
}
...
async function getProducts() {
    ...
    try {
        ...
        defaultCodes.result.forEach((prodList, k) => {
            console.log(`program Name: ', ${prodList.customerProgramName} \n programId: ', ${prodList.programId}`);
            ...
            document.querySelectorAll(".products-wrapper .products-boxes .product-card-spz").forEach((viewProd, i) => {
                ...
                if (prodList.customerProgramName == prName) {
                    console.log('viewProd: ', viewProd.getAttribute('data-targettitle'))
                    viewProd.setAttribute('data-program-id', prodList.programId);
                }
            });
        });
    } catch (error) {
